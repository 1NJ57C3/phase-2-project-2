...
function EditTaskCard({ id, task, setEditList, list}) {
    ...
    return(
        <div>
            ...
            <p onClick={handleCompletedChange}>Completed: {task.completed ? <button>✅</button> : <button>❌</button>}</p> {/* This behavior seems to happen because of the state's shared/piggybacking setup */}
        </div>
    )
}
